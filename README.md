...
| 2 | **Separating words** | A | [SeparatingWords.lean](OpenProblemsLab/SeparatingWords.lean) | Chase, STOC 2021: O(n^⅓ log⁷ n); 2025 improvement claim withdrawn; exact values n ≤ 18 (Tran, [AFL 2023](https://arxiv.org/abs/2309.02766)); sep(n) = 5 for 18 ≤ n ≤ 40 and sep(48) ≥ 6 (Bulatov, Karpova, Shur, Startsev, [E-JC 2017](https://arxiv.org/abs/1609.03199)) | **In progress, see [Results](#results).** BKSS length-48 identity formalized from their proof (sep(48) ≥ 6 in Lean); N(5) in [40, 47] open; log n vs n^⅓ gap wide open |
...
separating-words lower bound"). The later identity search (2026-09-13, logged in
[PREPRINT-OUTLINE.md](computations/separating_words/PREPRINT-OUTLINE.md) section 6b) found
earlier Lean work that GitHub code search does not index: Nicol's `jn1z/FurtherRemarks`
(2026-08-31) proves the unary lcm identity for DFAs and a separating-words lower bound.
Added 2026-09-13: `not_separates_bkss` transcribes the proof
...
`lt_sep_of_not_suffStates` gives `six_le_sep_48`: sep(48) >= 6 in Lean. That search found no
formal BKSS identity in the places it lists, which is its whole scope; no novelty is claimed
for these.
