...
| 6 | Antimagic | AM-4 paths in Lean | queued (construction pinned) | left-to-right labeling is antimagic iff m EVEN (collision at v=(m−1)/2 when m odd); swapping the last two labels fixes odd m — two-case construction verified for all m ≤ 400. Lean proof needs a parity split + path incidence sets |
...
| 9 | Odd covering | OC-3 SAT over survivors | done (partial + barrier) | L ≤ 20,000: 1,513 density-dead, 2 SAT-refuted (945, 2205), 41 undecided at 2e6 conflicts, 0 coverings ⇒ any odd covering with lcm ≤ 20k has lcm among 41 named shapes. Budget is the wall (85 s → 984 s per shape); 10× budget ≈ 1–2 machine-days [FORGE]; OC-4 = CRT-aware search or ILP |
